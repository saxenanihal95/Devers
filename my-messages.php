...
<?php
	$messages = DB::query('SELECT messages.*,users.username FROM messages,users WHERE (receiver=:reciever OR sender=:sender) AND users.id=messages.sender',array(':reciever'=>$userid,':sender'=>$userid));
